...
cPathCOPE = reshape(cellfun(@(s) PathUnsplit(DirAppend(strDirData, 'functional', s,...
    'feat_cat', 'stats'),'cope1','nii.gz'), cSubject, 'uni', false),[],1);
cPathCOPERep = repmat(cPathCOPE, 1, numel(cMaskName));

% Mean the results for each mask
cMaskMean = cellfun(@NIfTIMaskMean, cPathCOPERep, cMaskFlat, 'uni', false);
